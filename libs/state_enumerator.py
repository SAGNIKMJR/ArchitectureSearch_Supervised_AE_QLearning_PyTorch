...
class StateEnumerator:
    ...
    def enumerate_state(self, state, q_values):
        ...
        if state.terminate == 0:
            ...
            if state.layer_depth < self.layer_limit:
                ...
                # if (state.layer_type in ['start', 'conv', 'pool']):      # uncomment for include 'pool'
                if (state.layer_type in ['start', 'conv']):  
                    for depth in self.ssp.possible_conv_depths:
                        for filt in self._possible_conv_sizes(state.image_size):
                            # print('image size :{}'.format(self._calc_new_image_size(state.image_size, filt, 1)))
                ...
                #         for stride in self._possible_pool_strides(filt):                      # uncomment to include pool
                ...
                if (self.ssp.allow_fully_connected(state.image_size)
                    # and state.layer_type in ['start', 'conv', 'pool']):       # uncomment to include pool
                    and state.layer_type in ['start', 'conv']):
    ...
    def state_action_transition(self, start_state, action):
        ...
        '''
        # if action.layer_type == 'pool' or \                   # uncomment to include pool
        # if (action.layer_type == 'conv' and self.ssp.conv_padding == 'VALID'):
